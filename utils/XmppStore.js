...
class XmppStore {
  ...
  saveInterpretation( interpretation ) {
    if( !this.interpretations[interpretation.url] ) {
      this.interpretations = Object.assign({}, this.interpretations, {
        [interpretation.url]: {
          id: interpretation.id, name: interpretation.name,
  ...
  fileReceived( {from, uri} ) {
    ...
    if( !this.conversation[from_name] ) {
      ...
      this.conversation[from_name].chat.unshift({
        own: false,
        text: uri,
        date: dateFormat(date, "dd.mmm.yyyy"),
        time: dateFormat(date, "HH:MM"),
  ...
  sentFileinChat( image ) {
    ...
    if( !this.conversation[this.remote] ) {
      ...
      this.conversation[this.remote].chat.unshift({
        own: true,
        text: image,
        date: dateFormat(date, "dd.mmm.yyyy"),
        time: dateFormat(date, "HH:MM"),
  ...
  onMucMessage( {time, from, message} ) {
    ...
    if( !this.mucConversation[muc] ) {
      this.mucConversation = Object.assign({}, this.mucConversation, {
        [muc]: {
          chat: [{
            own: own,
            text: message,
      ...
      this.mucConversation[muc].chat.unshift({
        own: own,
        text: message,
        from: from_name,
        date: dateFormat(date, "dd.mmm.yyyy"),
